...
def verify_contact_added(db, contact_list, new_contact, app, check_ui):
    ...
    if check_ui:
        assert sorted(new_contacts, key=AddNew.id_or_max) == sorted(app.contact.get_contact_list(), key=AddNew.id_or_max)
...
def verify_contact_deleted(db, non_empty_contact_list, random_contact, app, check_ui):
    ...
    if check_ui:
        assert sorted(new_contacts, key=AddNew.id_or_max) == sorted(app.contacts.get_contact_list(), key=AddNew.id_or_max)
...
def contact_modify(lastname, firstname, address, homephone, mobilephone, workphone, secondaryphone, email, email2, email3):
    return AddNew(my_l_name=lastname, my_f_name=firstname, work_address=address,
                   my_h_telefon=homephone, my_mobile=mobilephone, my_work_telefon=workphone, my_secondary_phone=secondaryphone,
